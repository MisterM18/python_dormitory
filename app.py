...
def index():
    ...
    return render_template('index.html', items=items, item_urls=item_urls, total_pages=total_pages, current_page=page)
...
def other_page(item_id):
    ...
    db = client['DormitoryDB']
    collection = db['DDB']
    item = collection.find_one({'_id': ObjectId(item_id)})  # ใช้ ObjectId เพื่อแปลง item_id เป็น ObjectId
